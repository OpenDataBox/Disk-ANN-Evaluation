...
int aisaqPQReader::init_common(const char *pq_file_path, bool rearranged)
{
    ...
    // Try to detect block size from device
    m_block_size = 0;
    char device_path[40];
    ...
    // Fallback: use default block size if detection failed
    if (m_block_size == 0) {
        // Default to 4KB block size (common for most filesystems)
        m_block_size = 4096;
        std::cerr << "Warning: Could not detect block size, using default " << m_block_size << " bytes" << std::endl;
    }
