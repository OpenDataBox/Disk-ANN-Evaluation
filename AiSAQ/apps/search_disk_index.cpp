                      ...
                      const struct diskann::aisaq_search_config &aisaq_search_config)
{
    ...
    int res = _pFlashIndex->load(num_threads, index_path_prefix.c_str(), &aisaq_search_config);
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    diskann::cout << std::setw(6) << "L" << std::setw(12) << "Beamwidth" << std::setw(16) << "QPS" << std::setw(16)
                  ...
                  << "Mean IO (us)" << std::setw(16) << "CPU (s)" << std::setw(16) << "Mem Peak (MB)" << std::setw(16)
                  << "Mem Avg (MB)" << std::setw(16) << "PQ Dists" << std::setw(16) << "Exact Dists";
    if (calc_recall_flag)
    {
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        // Memory statistics
        auto mean_mem_peak = diskann::get_mean_stats<double>(
            stats, query_num, [](const diskann::QueryStats &stats) { 
                return (double)stats.memory_usage_peak / (1024.0 * 1024.0); // Convert to MB
            });
        
        auto mean_mem_avg = diskann::get_mean_stats<double>(
            stats, query_num, [](const diskann::QueryStats &stats) { 
                return ((double)stats.memory_usage_start + (double)stats.memory_usage_end) / 2.0 / (1024.0 * 1024.0); // Convert to MB
            });

        // Distance computation statistics
        auto mean_pq_dists = diskann::get_mean_stats<uint32_t>(
            stats, query_num, [](const diskann::QueryStats &stats) { return stats.n_pq_dists; });
        
        auto mean_exact_dists = diskann::get_mean_stats<uint32_t>(
            stats, query_num, [](const diskann::QueryStats &stats) { return stats.n_exact_dists; });
        ...
        diskann::cout << std::setw(6) << L << std::setw(12) << optimized_beamwidth << std::setw(16) << qps
                      ...
                      << std::setw(16) << mean_io_us << std::setw(16) << mean_cpuus << std::setw(16) << mean_mem_peak
                      << std::setw(16) << mean_mem_avg << std::setw(16) << mean_pq_dists << std::setw(16) << mean_exact_dists;
        if (calc_recall_flag)
        {
...
int main(int argc, char **argv)
{
    ...
    if (aisaq) {
        ...
        if (aisaq_pq_read_io_engine == std::string("aio")) {
            ...
            aisaq_search_config.pq_io_engine = diskann::aisaq_pq_io_engine_uring;
        } else {
        ...
        while (aisaq_pq_cache_size_string != std::string("")) {
            ...
            if (aisaq_pq_cache_size_string.back() == '%') {
                ...
                aisaq_search_config.pq_cache_size_unit = diskann::aisaq_size_unit_milli_percent;
                break;
            }
            ...
            if (aisaq_pq_cache_size_string.back() == 'B' || aisaq_pq_cache_size_string.back() == 'b') {
                aisaq_search_config.pq_cache_size_unit = diskann::aisaq_size_unit_bytes;
                aisaq_pq_cache_size_string.pop_back();
            }
            /* handle G/M/K */
        ...
        if (aisaq_search_config.pq_read_page_cache_size > (AISAQ_SEARCH_PQ_READ_PAGE_CACHE_MAX_DRAM_MB * (1 << 20))) {
            std::cerr << "pq_read_page_cache_size must be <= " << AISAQ_SEARCH_PQ_READ_PAGE_CACHE_MAX_DRAM_MB << "MiB" << std::endl;
            return -1;
        }
    }
