...
// Helper function to format build time as "XmYY.ZZZs"
std::string format_build_time(double seconds)
{
...
void save_build_time(const std::string &index_path_prefix, double seconds)
{
    std::string time_file = index_path_prefix + "_build_time.txt";
    std::ofstream ofs(time_file);
    if (ofs.is_open())
    {
        ofs << format_build_time(seconds) << std::endl;
        ofs.close();
        diskann::cout << "Build time saved to: " << time_file << std::endl;
    }
    else
    {
        diskann::cerr << "Warning: Could not save build time to " << time_file << std::endl;
    }
}

int main(int argc, char **argv)
{
    // Record start time for build duration tracking
    ...
    std::string params = std::string(std::to_string(R)) + " " + std::string(std::to_string(L)) + " " +
                         ...
                         std::string(std::to_string(aisaq_inline_pq)) + " " +
                         std::string(std::to_string(aisaq_rearrange)) + " " +
                         std::string(std::to_string(aisaq_num_entry_points));
    ...
    try
    {
        ...
        if (label_file != "" && label_type == "ushort")
        {
            if (data_type == std::string("int8"))
                result = diskann::build_disk_index<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                         metric, use_opq, codebook_prefix, use_filters, label_file,
                                                         universal_label, filter_threshold, Lf);
            else if (data_type == std::string("uint8"))
                result = diskann::build_disk_index<uint8_t, uint16_t>(
                ...
                result = diskann::build_disk_index<float, uint16_t>(
                    data_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric, use_opq, codebook_prefix,
                    use_filters, label_file, universal_label, filter_threshold, Lf);
            else
            {
        ...
        else
        {
            if (data_type == std::string("int8"))
                ...
                result = diskann::build_disk_index<uint8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                          metric, use_opq, codebook_prefix, use_filters, label_file,
                                                          universal_label, filter_threshold, Lf);
            else if (data_type == std::string("float"))
                result = diskann::build_disk_index<float>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                        metric, use_opq, codebook_prefix, use_filters, label_file,
                                                        universal_label, filter_threshold, Lf);
            else
            {
        ...
        // Calculate and save total build time
        auto build_end_time = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> build_duration = build_end_time - build_start_time;
        double total_seconds = build_duration.count();
        
        diskann::cout << "========================================" << std::endl;
        diskann::cout << "Total build time: " << format_build_time(total_seconds) << std::endl;
        diskann::cout << "========================================" << std::endl;
        
        // Save build time to file
        if (result == 0)
        {
            save_build_time(index_path_prefix, total_seconds);
        }
