...
// Get current RSS (Resident Set Size) memory usage in bytes for Linux
inline size_t get_currentRSS()
{
#ifdef __linux__
    long vmsize = 0L, rss = 0L;
    FILE *fp = fopen("/proc/self/statm", "r");
    if (fp != nullptr)
    {
        // /proc/self/statm format: VmSize RSS Shared Text Lib Data Dt
...
{
#ifdef __linux__
    size_t rss = get_currentRSS();
    diskann::cout << message << " [RSS: " << rss * 1.0 / (1024.0 * 1024 * 1024) << " GB]" << std::endl;
#endif
}

inline size_t getMemoryUsage()
{
    return get_currentRSS();
}
