...
namespace pipeann {
  ...
  int build_merged_vamana_index(std::string base_file, pipeann::Metric _compareMetric, bool single_file_index,
                                unsigned L, unsigned R, unsigned num_threads, double sampling_rate, double ram_budget,
    ...
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
      ...
      std::string index_prefix_path = (pos != std::string::npos) ? mem_index_path.substr(0, pos)
      : mem_index_path;
  ...
  bool build_disk_index(const char *dataPath, const char *indexFilePath, const char *indexBuildParameters,
    ...
    size_t num_pq_chunks = 750;// calculate_num_pq_chunks(final_index_ram_limit, points_num, dim);
    ...
    if(1){// start to refine pq pivots
      ...
      int type = 1;
      if(type == 1){
        refine_pq_pivots_from_errors_toppercent_in_one_file<T>(normalized_file_path, pq_pivots_path, pq_pivots_refined_path, index_prefix_path + "_map.bin", pq_compressed_vectors_refined_path, 256, (uint32_t) num_pq_chunks, NUM_KMEANS, 0.20);
        // refine_pq_pivots_from_errors_toppercent<T>(normalized_file_path, pq_pivots_path, pq_pivots_refined_path, index_prefix_path + "_map.bin", pq_compressed_vectors_refined_path, 256, (uint32_t) num_pq_chunks, NUM_KMEANS, 0.20);
      }else if(type == 2){
        refine_pq_pivots_from_errors<T>(normalized_file_path, pq_pivots_path, pq_pivots_refined_path, pq_compressed_vectors_refined_path, 256, (uint32_t) num_pq_chunks, NUM_KMEANS, 0.20);
      }else if(type == 3){
        build_dual_pq_512_from_scratch<T>(normalized_file_path, pq_pivots_path, pq_pivots_1_path, pq_pivots_2_path, index_prefix_path + "_map.bin", pq_compressed_vectors_1_path, pq_compressed_vectors_2_path, 256, (uint32_t) num_pq_chunks, NUM_KMEANS);
      }
      // 双PQ策略
      // 质心合并策略
    ...
    std::string pq_compressed_vectors_path_2 =
        index_prefix_path + "_pq_compressed_2.bin";
    ...
    if(use_double_pq){
      ...
      // generate_pq_pivots(train_data, train_size, (uint32_t)dim, 256, (uint32_t)num_pq_chunks, NUM_KMEANS, pq_pivots_path_tmp);
      // LOG(INFO) << "generate_pq_pivots done";
      // generate_train_data<T>(normalized_file_path, train_data, train_size, 256, (uint32_t)num_pq_chunks, pq_pivots_path_tmp);
      LOG(INFO) << "generate_train_data done";
      generate_pq_pivots(train_data, train_size, (uint32_t)dim, 256, (uint32_t)num_pq_chunks, NUM_KMEANS, pq_pivots_path_2);
      LOG(INFO) << "generate_pq_pivots done";
      generate_pq_data_from_pivots<T>(shuffled_normalized_file_path, 256,
    ...
    LOG(INFO) << "Compressed data generated and written in: " << std::chrono::duration<double>(end - start).count()
              << "s.";
    start = std::chrono::high_resolution_clock::now();
    pipeann::build_merged_vamana_index<T>(normalized_file_path, _compareMetric, single_file_index, L, R, num_threads, p_val,
  ...
  bool build_disk_index_py(const char *dataPath, const char *indexFilePath, uint32_t R, uint32_t L, uint32_t M,
    ...
    LOG(INFO) << "Compressed data generated and written in: " << std::chrono::duration<double>(end - start).count()
              << "s.";
    start = std::chrono::high_resolution_clock::now();
    pipeann::build_merged_vamana_index<T>(normalized_file_path, _compareMetric, single_file_index, L, R, num_threads, p_val, M,
  ...
  template int build_merged_vamana_index<int8_t>(std::string base_file, pipeann::Metric _compareMetric,
                                                 bool single_file_index, unsigned L, unsigned R, unsigned num_threads,
  ...
  template int build_merged_vamana_index<float>(std::string base_file, pipeann::Metric _compareMetric,
                                                bool single_file_index, unsigned L, unsigned R, unsigned num_threads,
                                                double sampling_rate, double ram_budget, std::string mem_index_path,
                                                std::string medoids_path, std::string centroids_file, const char *tag_file);
