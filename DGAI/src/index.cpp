...
namespace pipeann {
  ...
  void Index<T, TagT>::load_from_disk_index(const std::string &filename) {
    ...
    constexpr uint64_t kSectorLen = SECTOR_LEN;
    char *buf;
...
#pragma omp parallel for schedule(static, 65536)
  ...
  void Index<T, TagT>::link(Parameters &parameters) {
    ...
    unsigned link_threads = num_threads == 0 ? 128 : (num_threads > 128 ? 128 : num_threads);
    omp_set_num_threads(link_threads);
    ...
    using PageArrTopo = std::vector<uint32_t>;
    using PageArrCoord = std::vector<uint32_t>;
    PageArrTopo topo_tmp(ntopo_per_sector, kInvalidID);
    PageArrCoord coord_tmp(ncoord_per_sector, kInvalidID);
    ...
    auto ensure_topo_page = [&](uint32_t pid) {
      if (pid >= page_layout_topo.size()) {
        page_layout_topo.resize(static_cast<size_t>(pid) + 1, topo_tmp);
      }
    };
    auto ensure_coord_page = [&](uint32_t pid) {
      if (pid >= page_layout_coord.size()) {
        page_layout_coord.resize(static_cast<size_t>(pid) + 1, coord_tmp);
      }
    };
    id2loc_topo[_ep] = 0;
    ...
    uint32_t K = 3;
    std::vector<std::vector<unsigned>> layout_neighbors;
    if (build_topo_reorder || build_coord_reorder) {
      layout_neighbors.resize(n_vecs_to_visit);
    }
#pragma omp parallel for schedule(dynamic)
    for (int64_t node = 0; node < n_vecs_to_visit; node++) {
      ...
      if (build_topo_reorder || build_coord_reorder) {
        layout_neighbors[node] = pruned_list;
      }
    ...
    if (build_topo_reorder || build_coord_reorder) {
      for (int64_t node = 0; node < n_vecs_to_visit; node++) {
        const std::vector<unsigned> &pruned_list = layout_neighbors[node];

        if(build_topo_reorder)
        {
          uint32_t target_loc_coord = kInvalidID;
          uint32_t anchor_coord = kInvalidID;
          ...
          if(target_loc_coord != kInvalidID) {// no split
          ...
          } else if (anchor_coord == kInvalidID) {
            uint32_t new_pid = num_sectors_coord++;
            ensure_coord_page(new_pid);
            PageArrCoord &v = page_layout_coord[new_pid];
            v[0] = node;
            id2loc_coord[node] = new_pid * ncoord_per_sector;
          } else {
            uint32_t new_pid = num_sectors_coord++;
            ensure_coord_page(new_pid);
            ...
            uint32_t loc = id2loc_coord[anchor_coord];
            if (loc == kInvalidID) {
              LOG(ERROR) << "id " << anchor_coord << " not found in id2loc_coord";
              crash();
            }
            ...
            if(!exit){
              ...
              for (auto id : ids) {
                ...
                for (auto w : layout_neighbors[id]){
              ...
              if(group_id[anchor_coord] == 1 && ids1.size() < ncoord_per_sector){
                ids1.push_back(node);
              } else {
                ids2.push_back(node);
              }
        ...
        if(build_coord_reorder)
        {
          ...
          uint32_t anchor_topo = kInvalidID;
          for (uint32_t i = 0; i < pruned_list.size(); i++){
            uint32_t id = pruned_list[i];
          ...
          if(target_loc_topo != kInvalidID) {// no split
          ...
          } else if (anchor_topo == kInvalidID) {
            uint32_t new_pid = num_sectors_topo++;
            ensure_topo_page(new_pid);
            PageArrTopo &v = page_layout_topo[new_pid];
            v[0] = node;
            id2loc_topo[node] = new_pid * ntopo_per_sector;
          } else {
            uint32_t new_pid = num_sectors_topo++;
            ensure_topo_page(new_pid);
            std::unordered_set<uint32_t> ids;
            std::vector<uint32_t> ids1, ids2;
            uint32_t loc = id2loc_topo[anchor_topo];
            if (loc == kInvalidID) {
              LOG(ERROR) << "id " << anchor_topo << " not found in id2loc_topo";
              crash();
            }
            ...
            if(!exit)
            {
              ...
              if(group_id[anchor_topo] == 1 && ids1.size() < ntopo_per_sector){
                ids1.push_back(node);
              } else {
                ids2.push_back(node);
              }
