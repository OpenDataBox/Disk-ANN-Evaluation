...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    float total_minus_io_us = (float) pipeann::get_mean_stats(
        stats, query_num, [](const pipeann::QueryStats &stats) { return stats.total_us - stats.io_us; });

    float pq_cmps = (float) pipeann::get_mean_stats(
        stats, query_num, [](const pipeann::QueryStats &stats) { return stats.pq_cmps; });
