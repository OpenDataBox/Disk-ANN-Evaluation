...
enum SearchMode { BEAM_SEARCH = 0, PAGE_SEARCH = 1, PIPE_SEARCH = 2, RERANK_SEARCH = 3, CORO_SEARCH = 4 };
namespace {
...
#define READ_UNSIGNED(stream, val) stream.read((char *) &val, sizeof(unsigned))
  ...
  class SSDIndex {
   ...
   public:
    ...
    // Must be >= ntopo_per_sector. OpenAI3072 with 16KB sectors uses 83 topo entries per sector.
    using PageArrTopo = std::array<uint32_t, 128>;
