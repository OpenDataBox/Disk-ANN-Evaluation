...
int search_memory_index(int argc, char** argv) {
  ...
  diskann::load_aligned_bin<unsigned>(gt_ids_bin, gt_ids, gt_num, gt_dim,
                                      gt_aligned_dim);
  if (gt_num != query_num) {
  ...
  diskann::Index<T> index(diskann::L2, data_file.c_str(), true);
...
/*    std::pair<uint32_t, uint32_t> q_stats = index.beam_search(
        query + i * query_dim, recall_at, L, beam_width, start_points,
        query_result_ids[test_id].data() + i * recall_at);*/
#pragma omp atomic
...
#pragma omp atomic
    ...
    float recall = calc_recall_set(query_num, gt_ids, gt_aligned_dim,
