...
void insert_kernel(T* data_load, diskann::Index<T, TagT>& index,
  ...
  std::cout << "Insertion thread" << std::endl;
  diskann::Timer timer;
...
#pragma omp parallel for num_threads(16)
  ...
  std::cout << "Insert time : " << time_secs << " s\n"
...
int build_incremental_index(const std::string& data_path, const unsigned L,
  ...
  typedef int             TagT;
  diskann::Index<T, TagT> index(diskann::L2, dim, num_points, 1, true, true,
                                false);
  ...
  std::cout << "Saved batch build" << std::endl;
  T*     query = NULL;
  ...
  std::cout << "Saved file post insertions" << std::endl;
  ...
  if (argc < 12) {
    std::cout << "Correct usage: " << argv[0]
              << " type[int8/uint8/float] data_file L R C alpha "
                 "num_rounds "
              << "save_graph_file query_file recall@ L1 L2....." << std::endl;
    exit(-1);
  }
