...
void run(const uint32_t base_count, const uint32_t num_mem_indices,
  ...
  std::cout << "Loaded " << npts << " pts x " << ndims << " dims\n";
  //  assert(npts >= base_count + num_mem_indices);

  std::vector<uint32_t> tags(npts);
  std::iota(tags.begin(), tags.end(), 0);

  std::cout << "Base Index : choosing " << base_count << " points\n";
  ...
  while (count++ < num_cycles) {
    ...
    float active_tags_sampling_rate = (float) ((std::min)(
        (1.0 * delete_count) / (1.0 * active_tags.size()), 1.0));
    ...
    float inactive_tags_sampling_rate = (float) ((std::min)(
        (1.0 * incr_count) / (1.0 * inactive_tags.size()), 1.0));
    ...
    std::cout << "Merge program will insert " << insert_vec.size()
    ...
    for (uint64_t i = 0; i < num_mem_indices; i++) {
      ...
      std::cout << "Index #" << i + 1 << " : choosing " << new_start - cur_start
                << " points\n";
    ...
    // write mem
    //  std::cout << "Dumping {all} \\ {deleted} set\n";
