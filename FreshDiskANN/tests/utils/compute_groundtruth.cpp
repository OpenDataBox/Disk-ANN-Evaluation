               ...
               const float *const queries)  // queries in Col major
{
  ...
  compute_l2sq(points_l2sq, points, (_s64) npoints, (_u32) dim);
  compute_l2sq(queries_l2sq, queries, (_s64) nqueries, (_u32) dim);

  size_t q_batch_size = (1 << 8);
...
template<typename T>
inline _u32 get_num_parts(const char *filename) {
  ...
  reader.close();
  _u32 num_parts =
      (_u32) ((npts_i32 % PARTSIZE) == 0 ? npts_i32 / PARTSIZE
...
inline void save_groundtruth_as_one_file(const std::string filename,
  ...
  writer.write((char *) &npts_i32, sizeof(int));
  writer.write((char *) &ndims_i32, sizeof(int));
  uint64_t size = 2 * npts * ndims * sizeof(unsigned) + 2 * sizeof(int);
...
int aux_main(int argc, char **argv) {
  ...
  if (argc == 7) {
    ...
    uint32_t     *all_tags = new uint32_t[npoints];
    std::ifstream tag_reader(argv[6]);
    uint32_t      cur_i = 0;
    uint32_t      cur_tag;
    while (tag_reader >> cur_tag) {
      all_tags[cur_i++] = cur_tag;
    }
    tag_reader.close();
    std::cout << "Loaded tags for " << cur_i << " points.\n";
    assert(cur_i == npoints);
