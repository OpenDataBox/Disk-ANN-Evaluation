...
#define MERGE_TH 40000  // 硬编码为 40000 点（20 batch * 2000 points/batch）
namespace diskann {
  ...
  class MergeInsert {
   public:
    // constructor
    MergeInsert(Parameters& parameters, size_t max_points, size_t dim,
         const std::string& mem_prefix, std::string disk_prefix_in, std::string disk_prefix_out, Distance<T>* dist);

    ~MergeInsert();

    // insertion function - insert into short_term_index
    int insert(const T* point, const TagT& tag, int & percentile = 0);

    void lazy_delete(const TagT& tag);
    void lazy_delete(tsl::robin_set<TagT> & delete_list);

    // search function - search both short_term_index and long_term_index and
    // return with top L candidate tags of the shard
    void search_sync(const T* query, const uint64_t K, const uint64_t search_L,
                     TagT* tags, float * distances, QueryStats * stats);

    void return_active_tags(tsl::robin_set<TagT>& active_tags);
   
   //continuously runs in background to check if mem index size has exceeded its threshold - triggers index switch and merge
    int trigger_merge();

    void final_merge();
    
    std::string ret_merge_prefix();
   protected:

    //call constructor to IndexMerger object
    void construct_index_merger();
    
    //call IndexMerger destructor to explicitly de-register threads
    void destruct_index_merger();

    //_active_index flag will be modified only inside this function
    void switch_index(); //function to atomically switch btw indices, makes older index inactive(read-only), saves it, makes new index active (r/w)
    ...
    // call merge on a IndexMerger object, only if index switching and saving is successful
    void merge();
   ...
   IndexMerger<T> *  _merger = nullptr;
    ...
    std::atomic_bool _switching_disk_prefixes = false; //wait if true, search when false
    std::atomic_bool _check_switch_index = false; //true when switch_index acquires _index_lock in writer mode, insert threads wait till it turns back to false
    std::atomic_bool _check_switch_delete = false; //true when switching between _deletion_sets, _delete_lock 
    ...
    std::shared_timed_mutex _clear_lock_0; //lock to prevent an index from being cleared when it is being searched  and vice versa
    std::shared_timed_mutex _clear_lock_1; //lock to prevent an index from being cleared when it is being searched  and vice versa
