...
namespace diskann {
  ...
  inline void load_truthset(const std::string &bin_file, uint32_t *&ids,
    ...
    size_t ids_only_file_size =
        npts * dim * sizeof(uint32_t) + 2 * sizeof(uint32_t);

    bool has_dists = true;
    if (actual_file_size != expected_actual_file_size &&
        actual_file_size != with_tags_actual_file_size) {
      // Check if file only contains IDs (no distances)
      if (actual_file_size == ids_only_file_size) {
        has_dists = false;
        std::cout
            << "Warning: Ground truth file contains only IDs, no distances. "
            << "Distance values will be set to 0." << std::endl;
      } else {
        std::stringstream stream;
        stream << "Error. File size mismatch. Actual size is "
               << actual_file_size << " while expected size is  "
               << expected_actual_file_size << " (with dists) or "
               << ids_only_file_size << " (IDs only)"
    ...
    if (has_dists) {
      ...
      // No distances in file, set all to 0
      memset(dists, 0, npts * dim * sizeof(float));
    }
  }
  ...
  template<typename T>
  inline void save_unaligned_bin(const std::string &filename, T *data,
                                 size_t npts, size_t ndims,
                                 size_t aligned_dim) {
    std::ofstream writer(filename, std::ios::binary | std::ios::out);
    //   std::cout << "Writing unaligned bin : " << filename.c_str() << "\n";
    int npts_i32 = (int) npts, ndims_i32 = (int) ndims;
  ...
  template<typename T>
  inline void load_aligned_data(const std::string bin_file, T *&data,
                                size_t &npts, size_t &dim,
                                const size_t &rounded_dim) {
    if (data == nullptr) {
      std::cout << "Memory was not allocated for " << data
  ...
  inline void load_aligned_bin(const std::string    &bin_file,
    ...
    T *ptr;
    load_aligned_bin(bin_file, ptr, npts, dim, rounded_dim);
    data.reset(ptr);
  }
