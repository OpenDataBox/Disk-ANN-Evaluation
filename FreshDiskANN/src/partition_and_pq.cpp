...
int generate_pq_pivots(const std::unique_ptr<float[]> &passed_train_data,
                       size_t num_train, unsigned dim, unsigned num_centers,
                       unsigned num_pq_chunks, unsigned max_k_means_reps,
                       std::string pq_pivots_path) {
  float *passed_train_ptr = passed_train_data.get();
  if (generate_pq_pivots(passed_train_ptr, num_train, dim, num_centers,
                         num_pq_chunks, max_k_means_reps, pq_pivots_path) != 0)
    return -1;
  //  passed_train_data.reset(passed_train_ptr);
  return 0;
}
...
int generate_pq_pivots(const float *passed_train_data, size_t num_train,
  ...
  if (file_exists(pq_pivots_path)) {
    ...
    if (file_dim == dim && file_num_centers == num_centers) {
      std::cout << "PQ pivot file exists. Not running the initialization"
                << std::endl;
      update_flag = true;
      //      return -1;
    } else {
      std::cout << "PQ pivot file exists but dimension mismatch. Over-writing "
                   "the data."
                << std::endl;
      update_flag = false;
    }
...
#pragma omp parallel for schedule(static, 65536)
    ...
    //    if (update_flag)
    //      k_means_reps = (std::ceil)((1.0 * max_k_means_reps) / 5.0);
...
int generate_pq_data_from_pivots(const std::string data_file,
  ...
  if (!file_exists(pq_pivots_path)) {
    ...
    if (numr != dim || numc != 1) {
      std::cout << "Error reading centroid file. dim = " << dim << std::endl;
