...
namespace diskann {
  ...
  int build_disk_index(const char *dataFilePath, const char *indexFilePath,
                       const char *PQFilePath,
                       const char *    indexBuildParameters,
                       diskann::Metric compareMetric,
                       const _u64 SECTOR_LEN) {
    std::stringstream parser;
    std::cout << "Sector Len: " << SECTOR_LEN << std::endl;
    ...
    if (param_list.size() != 7 && param_list.size() != 8 &&
      ...
      diskann::cout
          << "Correct usage of parameters is R (max degree) "
             ...
             "build_PQ_only : set true to build pq only"
             "CP (Compressing of PQ)"
    ...
    if (param_list.size() == 8 || param_list.size() == 9) {
      disk_pq_dims = atoi(param_list[7].c_str());
      use_disk_pq = true;
      if (disk_pq_dims == 0)
        use_disk_pq = false;
    }
    ...
    std::string index_pq_prefix_path(PQFilePath);
    std::string pq_pivots_path = index_pq_prefix_path + "_pq_pivots.bin";
    std::string pq_compressed_vectors_path =
        index_pq_prefix_path + "_pq_compressed.bin";
    ...
    std::string disk_pq_pivots_path =
        index_pq_prefix_path + "_disk.index_pq_pivots.bin";
    // optional, used if disk index must store pq data
    std::string disk_pq_compressed_vectors_path =
        index_pq_prefix_path + "_disk.index_pq_compressed.bin";
    ...
    bool build_pq_only = false;
    if (1 == atoi(param_list[5].c_str())) {
      build_pq_only = true;
    }

    unsigned n_PQ_code = (unsigned) atoi(param_list[6].c_str());

    diskann::cout << "Starting index build: R=" << R << " L=" << L
                  << " Indexing ram budget: " << indexing_ram_budget
                  << " T: " << num_threads
                  << " build_pq_only: " << build_pq_only
                  << "N PQ code: " << n_PQ_code << std::endl;
    ...
    if (n_PQ_code != 0) {
      num_pq_chunks = (size_t) std::ceil(dim * 1.0 / n_PQ_code);
    } else {
      std::cerr << "please define n_PQ_code" << std::endl;
    }
    
    // Force 64 chunks for msong dataset (dim=420)
    if (dim == 420) {
      num_pq_chunks = 64;
      diskann::cout << "[OVERRIDE] Detected msong dataset (dim=420), forcing num_pq_chunks=64" << std::endl;
    }
...
// This code should only be available in the "build" DLL.
    ...
    if (!build_pq_only) {
      ...
      if (!use_disk_pq) {
        diskann::create_disk_layout<T>(data_file_to_use.c_str(), mem_index_path,
                                      disk_index_path, "", SECTOR_LEN);
      } else {
        std::cout << "we don't support disk pq yet." << std::endl;
        exit(-1);
      }
    }
  ...
  template DISKANN_DLLEXPORT int build_disk_index<int8_t>(
      const char *dataFilePath, const char *indexFilePath, const char * PQFilePath,
