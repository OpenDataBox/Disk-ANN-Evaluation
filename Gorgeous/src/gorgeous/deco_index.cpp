...
namespace diskann {
  ...
  template<typename T>
  void DecoIndex<T>::load_hot_nodes_from_txt(const std::string& txt_path,
                                              std::vector<unsigned>& tags,
                                              size_t max_nodes) {
    diskann::cout << "Loading hot nodes from text file: " << txt_path << std::endl;
    std::ifstream reader(txt_path);
    if (!reader.is_open()) {
      diskann::cerr << "Failed to open hot nodes file: " << txt_path << std::endl;
      return;
    }
    
    unsigned node_id;
    size_t count = 0;
    while (reader >> node_id) {
      tags.push_back(node_id);
      count++;
    ...
    reader.close();
    diskann::cout << "Loaded " << tags.size() << " hot nodes from text file" << std::endl;
  }

  template<typename T>
  void DecoIndex<T>::load_static_cache(const std::string& cache_list_file,
                                       size_t max_nodes) {
    diskann::cout << "Loading static cache from file: " << cache_list_file << std::endl;
    std::ifstream reader(cache_list_file);
    if (!reader.is_open()) {
      diskann::cerr << "Failed to open cache list file: " << cache_list_file << std::endl;
      return;
    }
    
    // Read node IDs from cache list
    std::vector<unsigned> cache_nodes;
    unsigned node_id;
    size_t count = 0;
    while (reader >> node_id) {
      cache_nodes.push_back(node_id);
      count++;
      if (max_nodes > 0 && count >= max_nodes) {
        break;
      }
    ...
    reader.close();
    diskann::cout << "Read " << cache_nodes.size() << " nodes from cache list" << std::endl;
    
    // Populate static_cached_nodes_ set
    static_cached_nodes_.clear();
    for (unsigned nid : cache_nodes) {
      static_cached_nodes_.insert(nid);
    }
    
    // Determine which pages contain these nodes
    tsl::robin_set<unsigned> pages_to_load;
    for (unsigned nid : cache_nodes) {
      if (nid < id2page_.size()) {
        unsigned page_id = id2page_[nid];
        pages_to_load.insert(page_id);
      }
    }
    diskann::cout << "Static cache spans " << pages_to_load.size() << " pages" << std::endl;
    
    // Allocate memory for static cache pages
    size_t total_cache_size = pages_to_load.size() * GR_SECTOR_LEN;
    diskann::alloc_aligned((void**)&static_cache_buf_, total_cache_size, GR_SECTOR_LEN);
    ...
    std::vector<unsigned> page_list(pages_to_load.begin(), pages_to_load.end());
    size_t pages_loaded = 0;
    
    for (size_t i = 0; i < page_list.size(); i += MAX_N_SECTOR_READS) {
      read_reqs.clear();
      tsl::robin_map<char*, unsigned> buf_to_page;
      
      size_t batch_end = std::min(i + MAX_N_SECTOR_READS, page_list.size());
      for (size_t j = i; j < batch_end; j++) {
        unsigned page_id = page_list[j];
        char* page_buf = static_cache_buf_ + pages_loaded * GR_SECTOR_LEN;
        auto offset = static_cast<_u64>(page_id + 1) * GR_SECTOR_LEN;
        read_reqs.push_back(AlignedRead(offset, GR_SECTOR_LEN, page_buf));
        buf_to_page[page_buf] = page_id;
        pages_loaded++;
      }
      
      if (!read_reqs.empty()) {
        auto n_ops = io_manager->submit_read_reqs(read_reqs, index_fid, ctx);
        while (n_ops > 0) {
          int n_read = io_manager->get_events(ctx, 1, n_ops, tmp_bufs);
          n_ops -= n_read;
          for (int k = 0; k < n_read; k++) {
            char* buf = tmp_bufs[k];
            unsigned page_id = buf_to_page[buf];
            static_page_cache_[page_id] = buf;
          }
        }
      }
    }
    
    diskann::cout << "Static cache loaded: " << static_cached_nodes_.size() 
                  << " nodes across " << static_page_cache_.size() << " pages" << std::endl;
  }
