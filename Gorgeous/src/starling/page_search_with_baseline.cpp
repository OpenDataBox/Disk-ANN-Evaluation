#include <immintrin.h>
#include <cstdlib>
#include <cstring>
#include "logger.h"
#include "percentile_stats.h"
#include "pq_flash_index.h"
#include "timer.h"
#include "baseline_io_stats.h"

namespace diskann {
  
  template<typename T>
  void PQFlashIndex<T>::page_search_with_baseline(
      const T *query1, const _u64 k_search, const _u32 mem_L, const _u64 l_search, _u64 *indices,
      float *distances, const _u64 beam_width, const _u32 io_limit,
      const float use_ratio, QueryStats *stats, 
      const BaselineIOStats* baseline_stats, const _u64 query_id) {
    
    Timer query_timer, tmp_timer, part_timer, cpu_timer;
    
  
    std::vector<unsigned> accessed_exact_nodes_vec;  
    tsl::robin_set<unsigned> accessed_exact_nodes_set; 
    std::vector<unsigned> accessed_adj_nodes_vec;    
    tsl::robin_set<unsigned> accessed_adj_nodes_set;    
    
    
    tsl::robin_set<unsigned> cache_hit_nodes;  
    
    ThreadData<T> data = this->thread_data.pop();
    while (data.scratch.sector_scratch == nullptr) {
      this->thread_data.wait_for_push_notify();
      data = this->thread_data.pop();
    }

    if (beam_width > MAX_N_SECTOR_READS)
      throw ANNException("Beamwidth can not be higher than MAX_N_SECTOR_READS",
                         -1, __FUNCSIG__, __FILE__, __LINE__);

    // copy query to thread specific aligned and allocated memory
    float        query_norm = 0;
    const T *    query = data.scratch.aligned_query_T;
    const float *query_float = data.scratch.aligned_query_float;

    uint32_t query_dim = metric == diskann::Metric::INNER_PRODUCT ? this->data_dim - 1 : this->data_dim;

    for (uint32_t i = 0; i < query_dim; i++) {
      data.scratch.aligned_query_float[i] = query1[i];
      data.scratch.aligned_query_T[i] = query1[i];
      query_norm += query1[i] * query1[i];
    }

    if (metric == diskann::Metric::INNER_PRODUCT) {
      query_norm = std::sqrt(query_norm);
      data.scratch.aligned_query_T[this->data_dim - 1] = 0;
      data.scratch.aligned_query_float[this->data_dim - 1] = 0;
      for (uint32_t i = 0; i < this->data_dim - 1; i++) {
        data.scratch.aligned_query_T[i] /= query_norm;
        data.scratch.aligned_query_float[i] /= query_norm;
      }
    }

    IOContext &ctx = data.ctx;
    auto       query_scratch = &(data.scratch);

    query_scratch->reset();

    T *   data_buf = query_scratch->coord_scratch;
    _mm_prefetch((char *) data_buf, _MM_HINT_T1);

    char *sector_scratch = query_scratch->sector_scratch;
    _u64 &sector_scratch_idx = query_scratch->sector_idx;

    float *pq_dists = query_scratch->aligned_pqtable_dist_scratch;
    pq_table.populate_chunk_distances(query_float, pq_dists);

    float *dist_scratch = query_scratch->aligned_dist_scratch;
    _u8 *  pq_coord_scratch = query_scratch->aligned_pq_coord_scratch;

    std::vector<Neighbor> retset(l_search + 1);
    tsl::robin_set<_u64> &visited = *(query_scratch->visited);
    tsl::robin_set<unsigned> &page_visited = *(query_scratch->page_visited);
    unsigned cur_list_size = 0;

    std::vector<Neighbor> full_retset;
    full_retset.reserve(4096);
    
    // Select best medoid by comparing query with all medoids
    _u32 best_medoid = 0;
    float best_dist = (std::numeric_limits<float>::max)();
    for (_u64 cur_m = 0; cur_m < num_medoids; cur_m++) {
      float cur_expanded_dist = dist_cmp_float->compare(
          query_float, centroid_data + aligned_dim * cur_m,
          (unsigned) aligned_dim);
      if (cur_expanded_dist < best_dist) {
        best_medoid = medoids[cur_m];
        best_dist = cur_expanded_dist;
      }
    }

    auto compute_pq_dists = [this, pq_coord_scratch, pq_dists, stats](const unsigned *ids,
                                                            const _u64 n_ids,
                                                            float *dists_out) {
      auto pq_start = std::chrono::high_resolution_clock::now();
      search_utils::aggregate_coords(ids, n_ids, this->data, this->n_chunks,
                         pq_coord_scratch);
      search_utils::pq_dist_lookup(pq_coord_scratch, n_ids, this->n_chunks, pq_dists,
                       dists_out);
      auto pq_end = std::chrono::high_resolution_clock::now();
      if (stats != nullptr) {
        stats->pq_distance_time_us += std::chrono::duration<double, std::micro>(pq_end - pq_start).count();
      }
    };

    auto compute_exact_dists_and_push = [&](const char* node_buf, const unsigned id) -> float {
      T *node_fp_coords_copy = data_buf;
      tmp_timer.reset();
      memcpy(node_fp_coords_copy, node_buf, disk_bytes_per_point);
      
      auto exact_start = std::chrono::high_resolution_clock::now();
      float cur_expanded_dist = dist_cmp->compare(query, node_fp_coords_copy,
                                            (unsigned) aligned_dim);
      auto exact_end = std::chrono::high_resolution_clock::now();
      
      if (stats != nullptr) {
        stats->n_ext_cmps++;
        stats->exact_distance_time_us += std::chrono::duration<double, std::micro>(exact_end - exact_start).count();
      }
      full_retset.push_back(Neighbor(id, cur_expanded_dist, true));
      
      if (accessed_exact_nodes_set.insert(id).second) {
        accessed_exact_nodes_vec.push_back(id);
      }
      
      return cur_expanded_dist;
    };

    auto compute_and_push_nbrs = [&](const char *node_buf, unsigned node_id, unsigned& nk) {
      if (accessed_adj_nodes_set.insert(node_id).second) {
        accessed_adj_nodes_vec.push_back(node_id);
      }
      
      unsigned *node_nbrs = OFFSET_TO_NODE_NHOOD(node_buf);
      unsigned nnbrs = *(node_nbrs++);
      unsigned nbors_cand_size = 0;
      tmp_timer.reset();
      for (unsigned m = 0; m < nnbrs; ++m) {
        if (visited.find(node_nbrs[m]) != visited.end()) 
          continue;
        else {
          visited.insert(node_nbrs[m]);
          node_nbrs[nbors_cand_size++] = node_nbrs[m];
        }
      }
      if (nbors_cand_size) {
        tmp_timer.reset();
        compute_pq_dists(node_nbrs, nbors_cand_size, dist_scratch);
        if (stats != nullptr) {
          stats->n_cmps += (double) nbors_cand_size;
        }
        for (unsigned m = 0; m < nbors_cand_size; ++m) {
          const int nbor_id = node_nbrs[m];
          const float nbor_dist = dist_scratch[m];
          if (nbor_dist >= retset[cur_list_size - 1].distance &&
              (cur_list_size == l_search))
            continue;
          Neighbor nn(nbor_id, nbor_dist, true);
          auto     r = InsertIntoPool(retset.data(), cur_list_size, nn);
          if (cur_list_size < l_search) ++cur_list_size;
          if (r < nk) nk = r;
        }
      }
    };

    auto compute_and_add_to_retset = [&](const unsigned *node_ids, const _u64 n_ids) {
      compute_pq_dists(node_ids, n_ids, dist_scratch);
      for (_u64 i = 0; i < n_ids; ++i) {
        retset[cur_list_size].id = node_ids[i];
        retset[cur_list_size].distance = dist_scratch[i];
        retset[cur_list_size++].flag = true;
        visited.insert(node_ids[i]);
      }
    };

    tmp_timer.reset();
    if (mem_L) {
      std::vector<unsigned> mem_tags(mem_L);
      std::vector<float> mem_dists(mem_L);
      std::vector<T*> res = std::vector<T*>();
      mem_index_->search_with_tags(query, mem_L, mem_L, mem_tags.data(), mem_dists.data(), nullptr, res);
      compute_and_add_to_retset(mem_tags.data(), std::min((unsigned)mem_L,(unsigned)l_search));
    } else {
      compute_and_add_to_retset(&best_medoid, 1);
      
      
      if (accessed_exact_nodes_set.insert(best_medoid).second) {
        accessed_exact_nodes_vec.push_back(best_medoid);
      }
    }
    

    std::sort(retset.begin(), retset.begin() + cur_list_size);

    if (stats != nullptr) {
      stats->preprocess_us += (double) tmp_timer.elapsed();
    }
    
    unsigned num_ios = 0;
    unsigned k = 0;
    
    // Per-hop cache hit tracking
    size_t current_hop = 0;

    std::vector<unsigned> frontier;
    frontier.reserve(2 * beam_width);
    std::vector<std::pair<unsigned, char *>> frontier_nhoods;
    frontier_nhoods.reserve(2 * beam_width);
    std::vector<AlignedRead> frontier_read_reqs;
    frontier_read_reqs.reserve(2 * beam_width);
    std::vector<std::pair<unsigned, std::pair<unsigned, unsigned *>>>
        cached_nhoods;
    cached_nhoods.reserve(2 * beam_width);

    std::vector<unsigned> last_io_ids;
    last_io_ids.reserve(2 * beam_width);
    std::vector<char> last_pages(SECTOR_LEN * beam_width * 2);
    int n_ops = 0;

    while (k < cur_list_size && num_ios < io_limit) {
      unsigned nk = cur_list_size;
      frontier.clear();
      frontier_nhoods.clear();
      frontier_read_reqs.clear();
      cached_nhoods.clear();
      sector_scratch_idx = 0;
      
      _u32 marker = k;
      _u32 num_seen = 0;

      part_timer.reset();
      while (marker < cur_list_size && frontier.size() < beam_width &&
             num_seen < beam_width) {
        const unsigned pid = id2page_[retset[marker].id];
        if (page_visited.find(pid) == page_visited.end() && retset[marker].flag) {
          num_seen++;
          auto iter = nhood_cache.find(retset[marker].id);
          if (iter != nhood_cache.end()) {
            cached_nhoods.push_back(
                std::make_pair(retset[marker].id, iter->second));
            if (stats != nullptr) {
              stats->n_cache_hits++;
            }
          } else {
            frontier.push_back(retset[marker].id);
            page_visited.insert(pid);
          }
          retset[marker].flag = false;
        }
        marker++;
      }
      if (stats != nullptr) stats->dispatch_us += (double) part_timer.elapsed();

      part_timer.reset();
      if (!frontier.empty()) {
        if (stats != nullptr)
          stats->n_hops++;
        for (_u64 i = 0; i < frontier.size(); i++) {
          auto                    id = frontier[i];
          std::pair<_u32, char *> fnhood;
          fnhood.first = id;
          fnhood.second = sector_scratch + sector_scratch_idx * SECTOR_LEN;
          sector_scratch_idx++;
          frontier_nhoods.push_back(fnhood);
          frontier_read_reqs.emplace_back(
              (static_cast<_u64>(id2page_[id]+1)) * SECTOR_LEN, SECTOR_LEN,
              fnhood.second);
          if (stats != nullptr) {
            stats->n_ios++;
          }
          num_ios++;
        }
        n_ops = reader->submit_reqs(frontier_read_reqs, ctx);
        if (this->count_visited_nodes) {
#pragma omp critical
          {
            auto &cnt = this->node_visit_counter[retset[marker].id].second;
            ++cnt;
          }
        }
      }
      if (stats != nullptr) stats->read_disk_us += (double) part_timer.elapsed();

      part_timer.reset();
      for (size_t i = 0; i < last_io_ids.size(); ++i) {
        const unsigned last_io_id = last_io_ids[i];
        char    *sector_buf = last_pages.data() + i * SECTOR_LEN;
        const unsigned pid = id2page_[last_io_id];
        const unsigned p_size = gp_layout_[pid].size();
        
        struct VisCand { float dist; const char* buf; unsigned id; };
        std::vector<VisCand> vis_cand;
        vis_cand.reserve(p_size);

        cpu_timer.reset();
       
        for (unsigned j = 0; j < p_size; ++j) {
          const unsigned id = gp_layout_[pid][j];
          if (id == last_io_id) continue;  
          const char* node_buf = sector_buf + j * max_node_len;
          float dist = compute_exact_dists_and_push(node_buf, id);
          vis_cand.push_back({dist, node_buf, id});
        }
        if (stats != nullptr) stats->cpu_us += (double) cpu_timer.elapsed();
        
    
        unsigned vis_size = use_ratio * (p_size - 1);
        if (vis_size && vis_size != p_size - 1) {
          std::sort(vis_cand.begin(), vis_cand.end(),
                    [](const VisCand& a, const VisCand& b) { return a.dist < b.dist; });
        }

        cpu_timer.reset();
        for (unsigned j = 0; j < vis_size; ++j) {
          compute_and_push_nbrs(vis_cand[j].buf, vis_cand[j].id, nk);
        }
        if (stats != nullptr) stats->cpu_us += (double) cpu_timer.elapsed();
      }
      last_io_ids.clear();
      if (stats != nullptr) stats->page_proc_us += (double) part_timer.elapsed();

      part_timer.reset();
      size_t hop_cache_hits = cached_nhoods.size();
      size_t hop_total_accesses = cached_nhoods.size() + frontier.size();
      
      for (auto &cached_nhood : cached_nhoods) {
        auto id = cached_nhood.first;
        auto  global_cache_iter = coord_cache.find(cached_nhood.first);
        T *   node_fp_coords_copy = global_cache_iter->second;
        unsigned nnr = cached_nhood.second.first;
        unsigned* cnhood = cached_nhood.second.second;
        char node_buf[max_node_len];
        memcpy(node_buf, node_fp_coords_copy, disk_bytes_per_point);
        memcpy((node_buf + disk_bytes_per_point), &nnr, sizeof(unsigned));
        memcpy((node_buf + disk_bytes_per_point + sizeof(unsigned)), cnhood, sizeof(unsigned)*nnr);
        cpu_timer.reset();
        compute_exact_dists_and_push(node_buf, id);
        compute_and_push_nbrs(node_buf, id, nk);
        if (stats != nullptr) stats->cpu_us += (double) cpu_timer.elapsed();
        
        cache_hit_nodes.insert(id);
      }
      if (stats != nullptr) stats->cache_proc_us += (double) part_timer.elapsed();
      
      if (stats != nullptr && current_hop < QueryStats::MAX_HOP_STATS) {
        stats->per_hop_cache_hits[current_hop] = hop_cache_hits;
        stats->per_hop_total_accesses[current_hop] = hop_total_accesses;
        stats->actual_hops = current_hop + 1;
      }
      current_hop++;

      part_timer.reset();
      if (!frontier.empty()) {
        reader->get_events(ctx, n_ops);
      }
      if (stats != nullptr) stats->read_disk_us += (double) part_timer.elapsed();

      part_timer.reset();
      for (auto &frontier_nhood : frontier_nhoods) {
        char *sector_buf = frontier_nhood.second;
        unsigned pid = id2page_[frontier_nhood.first];
        memcpy(last_pages.data() + last_io_ids.size() * SECTOR_LEN, sector_buf, SECTOR_LEN);
        last_io_ids.emplace_back(frontier_nhood.first);

        for (unsigned j = 0; j < gp_layout_[pid].size(); ++j) {
          unsigned id = gp_layout_[pid][j];
          if (id == frontier_nhood.first) {
            char *node_buf = sector_buf + j * max_node_len;
            cpu_timer.reset();
            compute_exact_dists_and_push(node_buf, id);
            compute_and_push_nbrs(node_buf, id, nk);
            if (stats != nullptr) stats->cpu_us += (double) cpu_timer.elapsed();
          }
        }
      }
      if (stats != nullptr) stats->disk_proc_us += (double) part_timer.elapsed();

      if (nk <= k)
        k = nk;
      else
        ++k;
    }
    
    tmp_timer.reset();
    
    std::sort(full_retset.begin(), full_retset.end(),
              [](const Neighbor &left, const Neighbor &right) {
                return left.distance < right.distance;
              });

    _u64 t = 0;
    for (_u64 i = 0; i < full_retset.size() && t < k_search; i++) {
      if(i > 0 && full_retset[i].id == full_retset[i-1].id){
        continue;
      }
      indices[t] = full_retset[i].id;
      if (distances != nullptr) {
        distances[t] = full_retset[i].distance;
        if (metric == diskann::Metric::INNER_PRODUCT) {
          distances[t] = (-distances[t]);
          if (max_base_norm != 0)
            distances[t] *= (max_base_norm * query_norm);
        }
      }
      t++;
    }

    if (t < k_search) {
      diskann::cerr << "The number of unique ids is less than topk" << std::endl;
      exit(1);
    }

    if (baseline_stats != nullptr && baseline_stats->is_loaded() && stats != nullptr) {
      size_t num_pages_read = stats->n_ios;
      
      size_t exact_vector_bytes = this->disk_bytes_per_point;  
      size_t adj_list_bytes = (this->max_degree+1) * sizeof(unsigned);  
      
      auto disk_util = baseline_stats->calculate_disk_utilization(
          accessed_exact_nodes_vec, accessed_adj_nodes_vec, query_id, num_pages_read, 
          exact_vector_bytes, adj_list_bytes);
      
      stats->baseline_total_disk_bytes = disk_util.total_disk_bytes;
      stats->baseline_hit_disk_bytes = disk_util.hit_disk_bytes;
      

      if (disk_util.total_disk_bytes > 0) {
        stats->io_utilization = static_cast<float>(disk_util.hit_disk_bytes) / 
                                static_cast<float>(disk_util.total_disk_bytes);
      } else {
        stats->io_utilization = 0.0f;
      }
      
      stats->useful_io_nodes = accessed_exact_nodes_vec.size() + accessed_adj_nodes_vec.size();
      stats->total_io_nodes = stats->useful_io_nodes;  
    }
    
    if (stats != nullptr) {
      size_t exact_vector_bytes = this->disk_bytes_per_point;
      size_t adj_list_bytes = (this->max_degree+1) * sizeof(unsigned);
      size_t per_node_bytes = exact_vector_bytes + adj_list_bytes;
      
      tsl::robin_set<unsigned> all_accessed_nodes;
      for (auto node_id : accessed_exact_nodes_vec) {
        all_accessed_nodes.insert(node_id);
      }
      for (auto node_id : accessed_adj_nodes_vec) {
        all_accessed_nodes.insert(node_id);
      }
      
      size_t cache_hit_bytes = 0;
      for (auto node_id : all_accessed_nodes) {
        if (cache_hit_nodes.find(node_id) != cache_hit_nodes.end()) {
          cache_hit_bytes += per_node_bytes;
        }
      }
      
      size_t total_cached_nodes = max_cached + coord_cache.size();
      size_t cache_size = total_cached_nodes * per_node_bytes;
      
      stats->cache_hit_bytes = cache_hit_bytes;
      stats->cache_total_bytes = cache_size;
      
      if (cache_size > 0) {
        stats->cache_hit_rate = static_cast<float>(cache_hit_bytes) / static_cast<float>(cache_size);
      }
    }
    
    this->thread_data.push(data);
    this->thread_data.push_notify_all();

    if (stats != nullptr) {
      stats->total_us = (double) query_timer.elapsed();
      stats->postprocess_us = (double) tmp_timer.elapsed();
    }
  }
  
  template class PQFlashIndex<_u8>;
  template class PQFlashIndex<_s8>;
  template class PQFlashIndex<float>;

} // namespace diskann
