...
#include <cstring>
#include <cstdlib>
#include <iomanip> // Added for debug output formatting
...
namespace diskann {
  ...
  PQFlashIndex<T>::PQFlashIndex(std::shared_ptr<AlignedFileReader> &fileReader,
    ...
    std::cout << "SECTOR_LEN: " << diskann_sector_len << std::endl;
    this->SECTOR_LEN = diskann_sector_len;
...
#pragma omp critical
  ...
  void PQFlashIndex<T>::load_cache_list(std::vector<uint32_t> &node_list) {
    ...
    unsigned max_robin_map = 1000000;  // Increased from 500000 to support larger cache
    if (node_list.size() > max_robin_map) {
      for (_u64 i = max_robin_map; i < node_list.size(); i++) {
        node_list[i] = i - max_robin_map;
      }
      max_cached = node_list.size() - max_robin_map;
      nhood_array.resize(max_cached);
      coord_array.resize(max_cached);
      std::cout << max_cached << std::endl;
    }
    ...
    for (_u64 block = 0; block < num_blocks; block++) {
      ...
      for (auto &nhood : nhoods) {
        ...
        if (nhood.first < max_cached) {
          coord_array[nhood.first] = cached_coords;
        }
        else {
        ...
        if (nhood.first < max_cached) {
          nhood_array[nhood.first] = cnhood;
        }
        else {
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
      for (_s64 i = 0; i < (int64_t) sample_num; i++) {
        page_search(
              ...
              beamwidth, std::numeric_limits<_u32>::max(), 1, nullptr);
      }
    }else{
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
      for (_s64 i = 0; i < (int64_t) sample_num; i++) {
...
template<typename T>
  ...
  int PQFlashIndex<T>::load(uint32_t num_threads, const char *index_prefix,
    ...
    // =========================================================
    // [DEBUG START] Read-only mode: check file header, never modify logic
    // =========================================================
    std::cout << "[DEBUG_PASSIVE] Checking file: " << pq_table_bin << std::endl;
    std::ifstream debug_in(pq_table_bin, std::ios::binary);
    if (debug_in) {
        unsigned n=0, d=0;
        debug_in.read((char*)&n, 4);
        debug_in.read((char*)&d, 4);
        std::cout << "[DEBUG_PASSIVE] Header at Offset 0: N=" << n << ", Dim=" << d << std::endl;
        
        debug_in.seekg(4096, std::ios::beg);
        if (debug_in) {
            debug_in.read((char*)&n, 4);
            debug_in.read((char*)&d, 4);
            std::cout << "[DEBUG_PASSIVE] Header at Offset 4096: N=" << n << ", Dim=" << d << std::endl;
        }
        debug_in.close();
    }
    // =========================================================
    ...
    // Support dynamic number of PQ centroids (not just 256)
    diskann::cout << "PQ file has " << pq_file_num_centroids << " centroids." << std::endl;
...
// read index metadata
  ...
  void PQFlashIndex<T>::cached_beam_search(
      ...
      QueryStats *stats, const _u32 mem_L) {
    Timer                 query_timer, io_timer, cpu_timer, tmp_timer, part_timer;
    
    // Node access statistics variables (for saving to file)
    static std::map<_u64, std::vector<unsigned>> all_exact_data_nodes;  // <query_id, node list>
    static std::map<_u64, std::vector<unsigned>> all_adjacency_nodes;   // <query_id, node list>
    static std::map<unsigned, unsigned> exact_data_frequency;           // exact_data node access frequency
    static std::map<unsigned, unsigned> adjacency_frequency;            // adjacency list node access frequency
    static std::atomic<_u64> query_counter;                             // query counter (atomic operation)
    static std::mutex stats_mutex;                                      // mutex to protect static variables
    static bool file_written;                                           // flag to indicate if file has been written
    
    // Get current query ID (thread-safe)
    _u64 current_query_id = query_counter.fetch_add(1);
    
    // Nodes accessed by current query
    std::vector<unsigned> current_exact_data_nodes;
    std::vector<unsigned> current_adjacency_nodes;
    tsl::robin_set<unsigned> exact_data_set;  // for deduplication
    tsl::robin_set<unsigned> adjacency_set;   // for deduplication
    ...
    auto compute_dists = [this, pq_coord_scratch, pq_dists, stats](const unsigned *ids,  // ids
    ...
    while (k < cur_list_size && num_ios < io_limit) {
      ...
      while (marker < cur_list_size && frontier.size() < beam_width &&
        ...
        if (retset[marker].flag) {
          num_seen++;
          auto ptr = find_in_nhood_cache(retset[marker].id);
          // auto iter = nhood_cache.find(retset[marker].id);
          if (ptr.first != 1e9) {
            cached_nhoods.push_back(
                std::make_pair(retset[marker].id, ptr));
            if (stats != nullptr) {
              stats->n_cache_hits++;
            }
          } else {
...
#pragma omp critical
      ...
      if (!frontier.empty()) {
        ...
        if (stats != nullptr) {
          ...
          stats->total_io_time_us += (float) io_timer.elapsed();
        }
      }
      ...
      for (auto &cached_nhood : cached_nhoods) {
        ...
        T * node_fp_coords_copy = find_in_coord_cache(cached_nhood.first);
        ...
        // 记录exact_data节点
        if (exact_data_set.insert(cached_nhood.first).second) {
          current_exact_data_nodes.push_back(cached_nhood.first);
        }

        _u64      nnbrs = cached_nhood.second.first;
        unsigned *node_nbrs = cached_nhood.second.second;
        
        // 记录被访问邻居表的节点（不是邻居表里的节点）
        if (adjacency_set.insert(cached_nhood.first).second) {
          current_adjacency_nodes.push_back(cached_nhood.first);
        }
      ...
      for (auto &frontier_nhood : frontier_nhoods) {
        ...
        if (exact_data_set.insert(frontier_nhood.first).second) {
          current_exact_data_nodes.push_back(frontier_nhood.first);
        }
        
        unsigned *node_nbrs = (node_buf + 1);
        

        if (adjacency_set.insert(frontier_nhood.first).second) {
          current_adjacency_nodes.push_back(frontier_nhood.first);
        }
...

    {
      ...
      all_exact_data_nodes[current_query_id] = current_exact_data_nodes;
      all_adjacency_nodes[current_query_id] = current_adjacency_nodes;
      
      for (unsigned node_id : current_exact_data_nodes) {
        exact_data_frequency[node_id]++;
      }
      for (unsigned node_id : current_adjacency_nodes) {
        adjacency_frequency[node_id]++;
      }
      

      if (all_exact_data_nodes.size() == 1000 && !file_written) {
        file_written = true;  
        
        // Get output directory from environment variable, fallback to current directory
        const char* output_dir_env = std::getenv("BASELINE_OUTPUT_DIR");
        std::string output_dir = (output_dir_env != nullptr && strlen(output_dir_env) > 0) 
                                  ? std::string(output_dir_env) 
                                  : ".";
        
        const char* subdir = std::getenv("BASELINE_SUBDIR");
        if (subdir != nullptr && strlen(subdir) > 0) {
          output_dir = output_dir + "/" + std::string(subdir);
          // Create subdirectory
          std::string mkdir_cmd = "mkdir -p " + output_dir;
          system(mkdir_cmd.c_str());
        }
        
        // Save exact_data nodes accessed by each query (in query_id order)
        std::ofstream exact_data_file(output_dir + "/exact_data_nodes.txt");
        if (exact_data_file.is_open()) {
          // map is automatically sorted by key, so iteration is in query_id order
          for (const auto& pair : all_exact_data_nodes) {
            const auto& nodes = pair.second;
            exact_data_file << nodes.size();
            for (unsigned node_id : nodes) {
              exact_data_file << ", " << node_id;
            }
            exact_data_file << "\n";
          }
          exact_data_file.close();
        }
        
        // Save adjacency list nodes accessed by each query (in query_id order)
        std::ofstream adjacency_file(output_dir + "/adjacency_nodes.txt");
        if (adjacency_file.is_open()) {
          // map is automatically sorted by key, so iteration is in query_id order
          for (const auto& pair : all_adjacency_nodes) {
            const auto& nodes = pair.second;
            adjacency_file << nodes.size();
            for (unsigned node_id : nodes) {
              adjacency_file << ", " << node_id;
            }
            adjacency_file << "\n";
          }
        ...
        // Save exact_data node access frequency (sorted by frequency in descending order)
        std::vector<std::pair<unsigned, unsigned>> exact_freq_vec(
            exact_data_frequency.begin(), exact_data_frequency.end());
        std::sort(exact_freq_vec.begin(), exact_freq_vec.end(),
                  [](const auto& a, const auto& b) { return a.second > b.second; });
        
        std::ofstream exact_freq_file(output_dir + "/exact_data_frequency.txt");
        if (exact_freq_file.is_open()) {
          for (const auto& pair : exact_freq_vec) {
            exact_freq_file << pair.first << ", " << pair.second << "\n";
          }
          exact_freq_file.close();
        }
        
        // Save adjacency list node access frequency (sorted by frequency in descending order)
        std::vector<std::pair<unsigned, unsigned>> adj_freq_vec(
            adjacency_frequency.begin(), adjacency_frequency.end());
        std::sort(adj_freq_vec.begin(), adj_freq_vec.end(),
                  [](const auto& a, const auto& b) { return a.second > b.second; });
        
        std::ofstream adj_freq_file(output_dir + "/adjacency_frequency.txt");
        if (adj_freq_file.is_open()) {
          for (const auto& pair : adj_freq_vec) {
            adj_freq_file << pair.first << ", " << pair.second << "\n";
          }
          adj_freq_file.close();
        }
        
        // Clear data to prepare for next round of 1000 queries
        all_exact_data_nodes.clear();
        all_adjacency_nodes.clear();
        exact_data_frequency.clear();
        adjacency_frequency.clear();
        file_written = false;  // Reset flag to allow next round of writing
      }
    }
