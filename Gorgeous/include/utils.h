...
namespace diskann {
  ...
  inline void load_aligned_bin_impl(std::basic_istream<char>& reader,
    ...
    // Optimized batch reading: read in chunks to reduce I/O operations
    const size_t BATCH_SIZE = 10000;  // Process 10k points at a time
    std::unique_ptr<T[]> temp_buffer(new T[BATCH_SIZE * dim]);
    
    for (size_t batch_start = 0; batch_start < npts; batch_start += BATCH_SIZE) {
      size_t batch_end = std::min(batch_start + BATCH_SIZE, npts);
      size_t batch_size = batch_end - batch_start;
      
      // Read entire batch at once
      reader.read((char*) temp_buffer.get(), batch_size * dim * sizeof(T));
      
      // Copy and align each point in the batch
      for (size_t i = 0; i < batch_size; i++) {
        size_t src_offset = i * dim;
        size_t dst_offset = (batch_start + i) * rounded_dim;
        std::memcpy(data + dst_offset, temp_buffer.get() + src_offset, dim * sizeof(T));
        std::memset(data + dst_offset + dim, 0, (rounded_dim - dim) * sizeof(T));
      }
    }
    diskann::cout << " done." << std::endl;
  }
