...
#define NUM_PQ_CENTROIDS 256  // Default value, but will support dynamic values

namespace diskann {
  class FixedChunkPQTable {
    float* tables = nullptr;  // pq_tables = float array of size [num_centroids * ndims]
    _u64   ndims = 0;         // ndims = true dimension of vectors
    _u64   n_chunks = 0;
    _u64   num_centroids = 256;  // Actual number of centroids, default 256
   ...
   public:
    ...
    // Support dynamic number of PQ centroids
    this->num_centroids = nr;
    diskann::cout << "PQ table loaded with " << this->num_centroids 
                  << " centroids (dynamic support enabled)" << std::endl;
    ...
    this->n_chunks = nr - 1;
    diskann::cout << "Loaded PQ Pivots: #ctrs: " << this->num_centroids
                  << ", #dims: " << this->ndims
                  << ", #chunks: " << this->n_chunks << std::endl;

    // alloc and compute transpose (using dynamic num_centroids)
    tables_T = new float[this->num_centroids * this->ndims];
    for (_u64 i = 0; i < this->num_centroids; i++) {
      for (_u64 j = 0; j < this->ndims; j++) {
        tables_T[j * this->num_centroids + i] = tables[i * this->ndims + j];
      }
    }
  }
  ...
  void populate_chunk_distances(const float* query_vec, float* dist_vec) {
    memset(dist_vec, 0, this->num_centroids * n_chunks * sizeof(float));
    // chunk wise distance computation
    for (_u64 chunk = 0; chunk < n_chunks; chunk++) {
      // sum (q-c)^2 for the dimensions associated with this chunk
      float* chunk_dists = dist_vec + (this->num_centroids * chunk);
      for (_u64 j = chunk_offsets[chunk]; j < chunk_offsets[chunk + 1]; j++) {
        _u64         permuted_dim_in_query = rearrangement[j];
        const float* centers_dim_vec = tables_T + (this->num_centroids * j);
        for (_u64 idx = 0; idx < this->num_centroids; idx++) {
          double diff =
