...
namespace diskann {
  ...
  class DecoIndex {
   public:
    ...
    // Load hot nodes from text file (one node ID per line), can limit the number
    DISKANN_DLLEXPORT void load_hot_nodes_from_txt(const std::string& txt_path,
                                         std::vector<unsigned>& tags,
                                         size_t max_nodes = 0);
    ...
    // Load static cache (load high-frequency nodes from cache_list file)
    DISKANN_DLLEXPORT void load_static_cache(const std::string& cache_list_file,
                                             size_t max_nodes = 0);
    
    // Check if node is in static cache
    DISKANN_DLLEXPORT bool is_in_static_cache(unsigned node_id) const {
        return static_cached_nodes_.find(node_id) != static_cached_nodes_.end();
    }
    
    // Get page data from static cache
    DISKANN_DLLEXPORT char* get_static_cache_page(unsigned page_id) const {
        auto it = static_page_cache_.find(page_id);
        if (it != static_page_cache_.end()) return it->second;
        return nullptr;
    }
    ...
    // page_search_dup_graph with baseline statistics
    DISKANN_DLLEXPORT void page_search_dup_graph_with_baseline(
        const T *query, const _u64 query_num, const _u64 query_aligned_dim, const _u64 k_search, const _u32 mem_L,
        const _u64 l_search, std::vector<_u64>& indices_vec, std::vector<float>& distances_vec,
        const _u64 beam_width, const _u32 io_limit,
        const float pq_filter_ratio, const float emb_search_ratio, QueryStats *stats,
        const BaselineIOStats* baseline_stats);

    // HybridCache search with hybrid caching and two-phase strategy
    DISKANN_DLLEXPORT void hybridcache_search(
        const T *query, const _u64 query_num, const _u64 query_aligned_dim, const _u64 k_search, const _u32 mem_L,
        const _u64 l_search, std::vector<_u64>& indices_vec, std::vector<float>& distances_vec,
        const _u64 beam_width, const _u32 io_limit,
        const float pq_filter_ratio, const float emb_search_ratio, 
        const struct HybridCacheSearchConfig& hc_config,
        QueryStats *stats = nullptr, struct HybridCacheSearchStats *hc_stats = nullptr);

    // DynamicCache (In-Memory First) search
    DISKANN_DLLEXPORT void dynamic_cache_search(
        const T *query, const _u64 query_num, const _u64 query_aligned_dim, const _u64 k_search, const _u32 mem_L,
        const _u64 l_search, std::vector<_u64>& indices_vec, std::vector<float>& distances_vec,
        const _u32 io_limit, const float emb_search_ratio,
        const struct DynamicCacheSearchConfig& dc_config,
        QueryStats *stats = nullptr, struct DynamicCacheSearchStats *dc_stats = nullptr);
   ...
   private:
    ...
    // static cache for HybridCache (page cache for high-frequency nodes)
    tsl::robin_map<unsigned, char*> static_page_cache_;  // page_id -> page_data
    tsl::robin_set<unsigned> static_cached_nodes_;       // Set of node IDs in static cache
    char* static_cache_buf_ = nullptr;                   // Memory buffer for static cache
    size_t static_cache_num_pages_ = 0;                  // Number of pages in static cache
