...
void relayout(const char* indexname, const char* partition_name, const int mode,
  ...
  if (mode == GRAPH_REPLICA) {
    ...
    // Note: Removed unreasonable validation that required i == layout[i][0]
    // Graph partitioning is based on connectivity, not node ID ordering
    std::cout << "Graph replica layout validation passed." << std::endl;
  }
  for (unsigned i = 0; i < _partition_nums; i++) {
    if (i % 100000 == 0) {
    ...
    if (mode == GRAPH_REPLICA) {
      // Fix: Use actual node ID instead of partition number
      uint64_t index_offset = READ_SECTOR_OFFSET(layout[i][0]);
