...
int main(int argc, char **argv) {
  ...
  unsigned clustering_page_size;
  bool clustering_verbose;
  ...
  try {
    ...
    // Clustering parameters
    desc.add_options()("clustering", po::value<bool>(&use_clustering)->default_value(false), 
                       "Enable clustering similarity-based partitioning");
    desc.add_options()("similarity_threshold", po::value<float>(&similarity_threshold)->default_value(0.7f), 
                       "Similarity threshold for clustering (0.0-1.0)");
    desc.add_options()("distance_func", po::value<std::string>(&distance_func)->default_value("L2"), 
                       "Distance function for clustering: L2 or cosine");
    desc.add_options()("clustering_page_size", po::value<unsigned>(&clustering_page_size)->default_value(0), 
                       "Page size for clustering (0 = auto)");
    desc.add_options()("clustering_verbose", po::value<bool>(&clustering_verbose)->default_value(false), 
                       "Enable verbose logging for clustering");
  ...
  if (use_clustering) {
    std::cout << "Using clustering similarity-based partitioning" << std::endl;
    std::cout << "Similarity threshold: " << similarity_threshold << std::endl;
    std::cout << "Distance function: " << distance_func << std::endl;
    
    GP::ClusteringConfig config;
    config.similarity_threshold = similarity_threshold;
    config.distance_function = distance_func;
    config.page_size = clustering_page_size;
    config.verbose_logging = clustering_verbose;
    config.enable_parallel = true;
    config.num_threads = thead_nums;
    
    GP::ClusteringPartitioner clustering_partitioner(index_file.c_str(), config, data_type.c_str(), 
                                                use_disk, block_size, visual, freq_file, cut, 
                                                dist_replace_freq, m, in_sector_len, out_sector_len);
    
    clustering_partitioner.clustering_partition(gp_file.c_str());
    
    const auto& stats = clustering_partitioner.get_statistics();
    std::cout << "\nClustering completed successfully!" << std::endl;
    std::cout << "Total clusters: " << stats.total_clusters << std::endl;
    std::cout << "Cross-page edges: " << stats.cross_page_edges << std::endl;
    
  } else {
    // Use original partitioning method
