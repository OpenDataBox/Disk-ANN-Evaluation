...
//
// PageANN: Linux Asynchronous I/O
...
int LinuxAlignedFileReader::submit_reqs(std::vector<AlignedRead> &read_reqs, io_context_t &ctx)
{
    assert(this->file_desc != -1);
    ...
    {
        io_prep_pread(cb.data() + j, this->file_desc, read_reqs[j].buf, read_reqs[j].len, read_reqs[j].offset);
    }
...
{
    std::vector<io_event_t> evts(n_ops);
    auto ret = io_getevents(ctx, (int64_t)n_ops, (int64_t)n_ops, evts.data(), nullptr);
    if (ret != (int64_t)n_ops)
    {
