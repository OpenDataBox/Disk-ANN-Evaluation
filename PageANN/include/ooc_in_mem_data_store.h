...
namespace diskann
{
template <typename data_t> class InMemOOCDataStore : public AbstractDataStore<data_t>
{
  public:
    InMemOOCDataStore(const location_t capacity, const size_t dim, std::unique_ptr<Distance<data_t>> distance_fn);
    virtual ~InMemOOCDataStore();
    ...
    void look_cache_node(uint32_t node);

    void cleanup_cache();

    void set_nnodes_to_load(size_t n);
