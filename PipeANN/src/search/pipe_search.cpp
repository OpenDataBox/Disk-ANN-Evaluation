...
namespace pipeann {
  ...
  size_t SSDIndex<T, TagT>::pipe_search(const T *query1, const uint64_t k_search, const uint32_t mem_L,
    ...
    auto compute_exact_dists_and_push = [&](const DiskNode<T> &node, const unsigned id) -> float {
      ...
      if (stats != nullptr) {
        stats->n_exact_cmps++;  // This is exact distance computation
      }
    ...
    auto compute_and_push_nbrs = [&](DiskNode<T> &node, unsigned &nk) {
      ...
      if (nbors_cand_size) {
        ...
        for (unsigned m = 0; m < nbors_cand_size; ++m) {
          ...
          if (stats != nullptr) {
            stats->n_cmps++;
            stats->n_pq_cmps++;  // This is PQ distance computation
          }
    ...
    auto io_time_st = std::chrono::high_resolution_clock::now();  // Start timing from first IO
    ...
    auto io_time_ed = std::chrono::high_resolution_clock::now();  // End timing after all IOs complete
    if (stats != nullptr) {
