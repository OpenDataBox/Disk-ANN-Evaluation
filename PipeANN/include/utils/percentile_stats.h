...
namespace pipeann {
  struct QueryStats {
    ...
    double n_cmps = 0;          // # cmps (total distance computations)
    double n_pq_cmps = 0;       // # PQ distance computations
    double n_exact_cmps = 0;    // # exact distance computations
    double n_cache_hits = 0;    // # cache_hits
    double n_hops = 0;          // # search hops
    double n_current_used = 0;  // # force return for latency limit
    double rss_kb = 0;          // current RSS in KB
  };
  ...
  inline double get_percentile_stats(QueryStats *stats, uint64_t len, float percentile,
    ...
    uint64_t idx = std::min((uint64_t)(percentile * ((float)len)), len - 1);
