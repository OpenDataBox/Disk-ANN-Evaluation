...
// Helper function to get current RSS in KB
static double get_current_rss_kb() {
...
// Helper function to get index disk size in MB
static double get_index_disk_size_mb(const std::string &index_prefix) {
  struct stat st;
...
#pragma omp parallel for schedule(dynamic, 1)
      for (int64_t i = 0; i < (int64_t) query_num; i++) {
        _pFlashIndex->pipe_search(query + (i * query_dim), (uint64_t) recall_at, mem_L, (uint64_t) L,
                                  ...
                                  stats + i);
        stats[i].rss_kb = get_current_rss_kb();
      }
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    float mean_io_us = (float) pipeann::get_mean_stats(stats, query_num,
                                                       [](const pipeann::QueryStats &stats) { return stats.io_us; });

    float mean_pq_cmps = (float) pipeann::get_mean_stats(stats, query_num,
                                                         [](const pipeann::QueryStats &stats) { return stats.n_pq_cmps; });

    float mean_exact_cmps = (float) pipeann::get_mean_stats(stats, query_num,
                                                            [](const pipeann::QueryStats &stats) { return stats.n_exact_cmps; });

    float mean_total_cmps = (float) pipeann::get_mean_stats(stats, query_num,
                                                            [](const pipeann::QueryStats &stats) { return stats.n_cmps; });

    float mean_rss_kb = (float) pipeann::get_mean_stats(stats, query_num,
                                                        [](const pipeann::QueryStats &stats) { return stats.rss_kb; });

    float peak_rss_kb = (float) pipeann::get_percentile_stats(stats, query_num, 1.0f,
                                                              [](const pipeann::QueryStats &stats) { return stats.rss_kb; });
    ...
    if (output) {
      ...
      std::cout << std::setw(6) << L << std::setw(12) << beamwidth << std::setw(12) << qps << std::setw(12)
                ...
                << mean_ios << std::setw(12) << mean_io_us << std::setw(12) << mean_pq_cmps << std::setw(12) 
                << mean_exact_cmps << std::setw(12) << mean_total_cmps << std::setw(12) << (mean_rss_kb / 1024.0) 
                << std::setw(12) << (peak_rss_kb / 1024.0);
      if (calc_recall_flag) {
  ...
  // Print index disk size
  double index_size_mb = get_index_disk_size_mb(index_prefix_path);
  std::cout << "Index Disk Size: " << std::fixed << std::setprecision(2) << index_size_mb << " MB" << std::endl;
  std::cout << std::endl;
  ...
  std::cout << std::setw(6) << "L" << std::setw(12) << "I/O Width" << std::setw(12) << "QPS" << std::setw(12)
            ...
            << std::setw(12) << "Mean IO(us)" << std::setw(12) << "PQ Cmps" << std::setw(12) << "Exact Cmps" 
            << std::setw(12) << "Total Cmps" << std::setw(12) << "Avg RSS(MB)" << std::setw(12) << "Peak RSS(MB)"
            << std::setw(12);
  if (calc_recall_flag) {
