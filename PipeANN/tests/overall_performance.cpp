...
template<typename T, typename TagT>
double insertion_kernel(T *data_load, pipeann::DynamicSSDIndex<T, TagT> &sync_index, std::vector<TagT> &insert_vec,
                        size_t aligned_dim) {
  pipeann::Timer timer;
...
void update(const std::string &data_bin, const unsigned L_disk, int step, const unsigned nodes_to_cache,
  ...
  int batch = step;  // use step as the number of batches
  int inMemorySize = 0;
  std::future<void> merge_future;
  uint64_t index_npts = sync_index._disk_index->meta_.npoints;  // initial number of points (e.g., 800K)
  uint64_t vecs_per_step = 2000;  // fixed: insert/delete 2000 points per batch
  
  // Statistics for update time
  std::vector<double> update_times;  // time for each batch update (in seconds)
  double total_update_time = 0.0;
  
  // Statistics for merge time
  std::vector<double> merge_times;  // time for each merge (in seconds)
  ...
  for (int i = 0; i < batch; i++) {
    ...
    // Start timing for this batch update
    auto batch_start = std::chrono::high_resolution_clock::now();
    
    std::vector<unsigned> insert_vec;
    std::vector<unsigned> delete_vec;

    /**Prepare for update - sliding window*/
    // Delete from the beginning: [i*vecs_per_step, (i+1)*vecs_per_step)
    // Insert from the end: [index_npts + i*vecs_per_step, index_npts + (i+1)*vecs_per_step)
    uint64_t delete_start = i * vecs_per_step;
    uint64_t insert_start = index_npts + i * vecs_per_step;
    get_trace<T, TagT>(data_bin, delete_start, insert_start, vecs_per_step, delete_vec, insert_vec, data_load);

    std::future<double> insert_future = std::async(std::launch::async, insertion_kernel<T, TagT>, data_load.data(),
    ...
    // End timing for this batch update
    auto batch_end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> batch_duration = batch_end - batch_start;
    double batch_time_sec = batch_duration.count();
    update_times.push_back(batch_time_sec);
    total_update_time += batch_time_sec;
    
    std::cout << "Batch " << i << " update time: " << batch_time_sec << " seconds" << std::endl;

    inMemorySize += insert_vec.size();

    // Update the current window position for ground truth
    res = (i + 1) * vecs_per_step;  // current position in the sliding window
    std::cout << "Search after update, current window position: " << res << std::endl;
    ...
    if (i == batch - 1) {
      ...
      std::cout << "\n========================================" << std::endl;
      std::cout << "Update Statistics Summary" << std::endl;
      std::cout << "========================================" << std::endl;
      std::cout << "Total batches: " << batch << std::endl;
      std::cout << "Vectors per batch: " << vecs_per_step << std::endl;
      std::cout << "Total vectors updated: " << (batch * vecs_per_step) << std::endl;
      std::cout << "Total update time: " << total_update_time << " seconds" << std::endl;
      std::cout << "Average update time per batch: " << (total_update_time / batch) << " seconds" << std::endl;
      std::cout << "Average throughput: " << (batch * vecs_per_step / total_update_time) << " vectors/second" << std::endl;
      
      // Calculate percentiles
      std::vector<double> sorted_times = update_times;
      std::sort(sorted_times.begin(), sorted_times.end());
      std::cout << "\nUpdate time percentiles:" << std::endl;
      std::cout << "  Min: " << sorted_times[0] << " seconds" << std::endl;
      std::cout << "  50th: " << sorted_times[(size_t)(0.5 * batch)] << " seconds" << std::endl;
      std::cout << "  90th: " << sorted_times[(size_t)(0.9 * batch)] << " seconds" << std::endl;
      std::cout << "  95th: " << sorted_times[(size_t)(0.95 * batch)] << " seconds" << std::endl;
      std::cout << "  99th: " << sorted_times[(size_t)(0.99 * batch)] << " seconds" << std::endl;
      std::cout << "  Max: " << sorted_times[batch - 1] << " seconds" << std::endl;
      std::cout << "========================================\n" << std::endl;
      
      // Print merge statistics
      if (merge_count > 0) {
        std::cout << "========================================" << std::endl;
        std::cout << "Merge Statistics Summary" << std::endl;
        std::cout << "========================================" << std::endl;
        std::cout << "Total merges: " << merge_count << std::endl;
        std::cout << "Total merge time: " << total_merge_time << " seconds" << std::endl;
        std::cout << "Average merge time: " << (total_merge_time / merge_count) << " seconds" << std::endl;
        
        // Calculate merge time percentiles
        std::vector<double> sorted_merge_times = merge_times;
        std::sort(sorted_merge_times.begin(), sorted_merge_times.end());
        std::cout << "\nMerge time percentiles:" << std::endl;
        std::cout << "  Min: " << sorted_merge_times[0] << " seconds" << std::endl;
        if (merge_count > 1) {
          std::cout << "  50th: " << sorted_merge_times[(size_t)(0.5 * merge_count)] << " seconds" << std::endl;
          std::cout << "  90th: " << sorted_merge_times[(size_t)(0.9 * merge_count)] << " seconds" << std::endl;
          std::cout << "  95th: " << sorted_merge_times[(size_t)(0.95 * merge_count)] << " seconds" << std::endl;
          std::cout << "  99th: " << sorted_merge_times[(size_t)(0.99 * merge_count)] << " seconds" << std::endl;
        }
        std::cout << "  Max: " << sorted_merge_times[merge_count - 1] << " seconds" << std::endl;
        std::cout << "========================================\n" << std::endl;
      }
      ...
      std::chrono::duration<double> merge_duration = merge_end - merge_start;
      double merge_time_sec = merge_duration.count();
      merge_times.push_back(merge_time_sec);
      total_merge_time += merge_time_sec;
      merge_count++;
      
      std::cout << "Merge " << merge_count << " completed in " << merge_time_sec << " seconds" << std::endl;
